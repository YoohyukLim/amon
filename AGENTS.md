...
Before project work, read the cairn rules below. If any referenced file is missing, run `cairn:doctor` or restart Codex with the cairn plugin enabled so project bootstrap can refresh `~/.cairn/rules/v1/**`.

## Cairn Runtime Contract

This section is inlined because Codex does not reliably expand referenced rule
paths during instruction assembly. Treat it as already loaded.
...
- Use qualified skills: `cairn:doctor`, `cairn:pickup`, `cairn:handoff`,
  `cairn:atlas-init`, `cairn:atlas-sync`, and `cairn:atlas-extract`.
- Codex marker hooks track `apply_patch` file intent only.
- Missing marker paths are drift evidence, not verified fact.
...
- ~/.cairn/rules/v1/codex/cairn-codex-runtime-paths.md
- ~/.cairn/rules/v1/codex/cairn-codex-agent-selection.md
- ~/.cairn/rules/v1/codex/cairn-codex-hook-behavior.md
- ~/.cairn/rules/v1/codex/cairn-codex-skill-entrypoints.md
